...
public:
	...
	bool Read(T& n, bool bOffset = true)
	{
		...
		//断言assert
		//错误日志
		CRCLog_Error("error, CRCStream::Read failed.");
		return false;
	}
	...
	uint32_t ReadArray(T* pArr, uint32_t len)
	{
		...
		CRCLog_Error("CRCStream::ReadArray failed.");
		return 0;
	}
...
//////Write
	...
	bool Write(T n)
	{
		...
		CRCLog_Error("CRCStream::Write failed.");
		return false;
	}
	template<typename T>
	bool WriteArray(T* pData, uint32_t len)
	{
		...
		CRCLog_Error("CRCStream::WriteArray failed.");
		return false;
	}
