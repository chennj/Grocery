...
CRCClientSWebSocket::handshake()
{
    ...
    {
        CRCLog_Error("CRCClientSWebSocket::handshake, not found Upgrade:websocket");
        return false;
    }
    ...
    if (!cKey)
    {
        CRCLog_Error("CRCClientSWebSocket::handshake, not found Sec-WebSocket-Key");
        return false;
    }
    ...
    CRCIO::SHA1_String((const unsigned char*)sKey.c_str(), sKey.length(), strSha1);

    std::string sKeyAccept = CRCIO::Base64Encode(strSha1, 20);
...
bool 
CRCClientSWebSocket::hasMsg()
{
    if (clientState_join == _clientState)
    {
        return CRCClientSHttp::hasMsg();
    }
...
void 
CRCClientSWebSocket::pop_front_msg()
{
...
CRCClientSWebSocket::hasMsgWS()
{
    ...
    //
    _wsh.opcode = static_cast<CRCIO::WebSocketOpcode>(data[0] & 0xF);
    //
    ...
    //
    if ((!_wsh.mask && CRCIO::opcode_PING != _wsh.opcode && CRCIO::opcode_PONG != _wsh.opcode)
        || CRCIO::opcode_CLOSE == _wsh.opcode)
    {
        onClose();
        return true;
    }
    ...
    {
        CRCLog_Error("CRCClientSWebSocket::hasMsgWS -> _wsh.header_size + _wsh.len > _recvBuff.buffSize()");
        onClose();
        return false;
    }
    //判断数据收完没有
...
char * 
CRCClientSWebSocket::fetch_data()
{
...
int 
CRCClientSWebSocket::writeHeader(CRCIO::WebSocketOpcode opcode, uint64_t len)
{
    uint8_t header[10] = {};
    ...
    if (SOCKET_ERROR == ret)
    {
        CRCLog_Error("CRCClientSWebSocket::writeHeader -> SendData -> SOCKET_ERROR");
    }
    return ret;
}

int 
CRCClientSWebSocket::writeText(const char* pData, int len)
{
    int ret = writeHeader(CRCIO::opcode_TEXT, len);
    if (SOCKET_ERROR != ret)
    {
        ret = SendData(pData, len);
        if (SOCKET_ERROR == ret)
        {
            CRCLog_Error("CRCClientSWebSocket::writeText -> SendData -> SOCKET_ERROR");
        }
    }
    return ret;
}
...
int 
CRCClientSWebSocket::ping()
{
    return writeHeader(CRCIO::opcode_PING, 0);
}

int 
CRCClientSWebSocket::pong()
{
