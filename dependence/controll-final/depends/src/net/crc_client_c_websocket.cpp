...
CRCClientCWebSocket::CRCClientCWebSocket(SOCKET sockfd, int sendSize, int recvSize) :
    CRCClientCHttp(sockfd, sendSize, recvSize)
{
...
void 
CRCClientCWebSocket::pop_front_msg()
{
...
CRCClientCWebSocket::hasMsgWS()
{
    ...
    //
    if (CRCIO::opcode_CLOSE == _wsh.opcode)
    {
        onClose();
        return true;
    }
    ...
    {
        CRCLog_Error("WebSocketClientC::hasMsgWS -> _wsh.header_size + _wsh.len > _recvBuff.buffSize()");
        onClose();
        return false;
    }
    //判断数据收完没有
...
char * 
CRCClientCWebSocket::fetch_data()
{
...
void 
CRCClientCWebSocket::do_mask(int len)
{
...
int 
CRCClientCWebSocket::writeHeader(CRCIO::WebSocketOpcode opcode, uint64_t len, bool mask, int32_t mask_key)
{
    uint8_t header[14] = {};
    ...
    if (SOCKET_ERROR == ret)
    {
        CRCLog_Error("WebSocketClientC::writeHeader -> SendData -> SOCKET_ERROR");
    }
    return ret;
}

int 
CRCClientCWebSocket::writeText(const char* pData, int len)
{
    int ret = writeHeader(CRCIO::opcode_TEXT, len, true, _mask_key);
    if (SOCKET_ERROR != ret)
    {
        ret = SendData(pData, len);
        if (SOCKET_ERROR == ret)
        {
            CRCLog_Error("WebSocketClientC::writeText -> SendData -> SOCKET_ERROR");
            return ret;
        }
        do_mask(len);
    }
...
int 
CRCClientCWebSocket::ping()
{
    return writeHeader(CRCIO::opcode_PING, 0, false, 0);
}

int 
CRCClientCWebSocket::pong()
{
    return writeHeader(CRCIO::opcode_PONG, 0, false, 0);
}

CRCIO::WebSocketHeader& 
CRCClientCWebSocket::WebsocketHeader()
{
    return _wsh;
}